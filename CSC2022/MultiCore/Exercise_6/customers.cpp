/*
g++ -o customers customers.cpp -std=c++14
*/
#include <chrono>
#include <fstream>
#include <iostream>
...
// This program *is badly designed and implemented on purpose*.
...
using Customerv = std::vector<Customer>;
using Customervv = std::vector<Customerv>;

Customerv gCustomers;
Customervv gCustomersAlphabetic(26);
...
int readCustomersData(const std::string filename) {
  ...
  while (!iFile.eof()) {
    ...
    Customer c(id++, data[0], data[1], data[2], data[3]);
    gCustomers.push_back(c);
  }
...
void fillCustomerDataAlphabetic() {
  ...
  for (auto & customer : gCustomers) {
    ...
    char initial = name[0];
    initial -= offset;
    gCustomersAlphabetic[(int)initial].push_back(customer);
  }
}

int main(int argc, char ** argv) {

  std::cout << "DISCLAIMER: This program *is badly designed "
            << "and implemented on purpose*. "
            << "The goal is to test performance profiling tools "
            << "and understand some most common "
